...
# 限制解压前文件最大大小：98MB (给根节点闭合标签留安全空间)
...
def merge_epg():
    if not os.path.exists(CONFIG_FILE):
        print(f"错误: 找不到配置文件 {CONFIG_FILE}")[cite: 2]
        return

    # 按 config.txt 顺序读取源[cite: 2]
    with open(CONFIG_FILE, "r", encoding="utf-8") as f:
        sources = [line.strip() for line in f if line.strip() and not line.startswith("#")]

    if not sources:
        print("警告: config.txt 中未找到有效的 EPG 源")[cite: 2]
        return

    os.makedirs(OUTPUT_DIR, exist_ok=True)[cite: 2]

    seen_channel_ids = set()   # 记录已添加的频道 ID[cite: 2]
    seen_programmes = set()    # 记录已添加的节目 (channel_id, start, stop)[cite: 2]
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f_out:
            ...
            for channel in root.findall("channel"):
                ...
                if channel_id and channel_id not in seen_channel_ids:
                    ...
                    if current_bytes + len(encoded_bytes) >= MAX_FILE_SIZE:
                        print("警告: 写入频道列表时已达到文件大小上限！")[cite: 2]
                        f_out.write("</tv>\n")
                        break
        ...
        # 优先按 config.txt 顺序写入节目列表 (仅限 UTC+8 今明两天)[cite: 2]
        is_truncated = False
        for root in parsed_sources:
            if is_truncated:
                break
            for programme in root.findall("programme"):
                ...
                if prog_key not in seen_programmes:
                    ...
                    # 精确控制解压前文件不超过 100MB[cite: 2]
                    if current_bytes + len(encoded_bytes) >= MAX_FILE_SIZE:
                        print("提示: 文件大小接近 100MB 限制，已截断后续节目。")[cite: 2]
                        is_truncated = True
                        break
    ...
    print(f"XML 文件已生成: {OUTPUT_FILE} ({xml_size_mb:.2f} MB)")[cite: 2]

    # 2. 生成 epg.xml.gz 压缩文件[cite: 2]
    with open(OUTPUT_FILE, 'rb') as f_in:
        with gzip.open(OUTPUT_GZ_FILE, 'wb', compresslevel=9) as f_out:
            shutil.copyfileobj(f_in, f_out)
            
    gz_size_mb = os.path.getsize(OUTPUT_GZ_FILE) / (1024 * 1024)
    print(f"GZ 压缩文件已生成: {OUTPUT_GZ_FILE} ({gz_size_mb:.2f} MB)")[cite: 2]
